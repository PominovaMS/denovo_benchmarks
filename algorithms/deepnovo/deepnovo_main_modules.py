...
def read_spectra(file_handle, data_format, spectra_locations):
    ...
    for location in spectra_locations:
        ...
        if data_format == "mgf":
            ...
            peptide_mass = (
                peptide_ion_mz * charge - charge * deepnovo_config.mass_H
            )
            ...
            if (
                abs(peptide_mass - sequence_mass)
        ...
        ) = process_spectrum(spectrum_mz, spectrum_intensity, peptide_mass)
        ...
        if (
            ...
            or deepnovo_config.FLAGS.direction == 2
        ):
        ...
        # backward
        if (
            deepnovo_config.FLAGS.direction == 1
            or deepnovo_config.FLAGS.direction == 2
        ):
        ...
        if not deepnovo_config.FLAGS.beam_search:
            ...
            if (
                ...
                for index in xrange(decoder_size):
                    ...
                    prefix_mass += deepnovo_config.mass_ID[
                        peptide_ids_forward[index]
                    ]
                    candidate_intensity = get_candidate_intensity(
                        spectrum_original_forward, peptide_mass, prefix_mass, 0
                    )
                    candidate_intensity_list_forward.append(
                        candidate_intensity
                    )
            ...
            if (
                ...
                for index in xrange(decoder_size):
                    ...
                    suffix_mass += deepnovo_config.mass_ID[
                        peptide_ids_backward[index]
                    ]
                    candidate_intensity = get_candidate_intensity(
                        spectrum_original_backward,
                        peptide_mass,
                    ...
                    candidate_intensity_list_backward.append(
                        candidate_intensity
                    )
    ...
    print(
        "  total peptide skipped by mass precision %d"
...
def read_random_stack(file_handle, data_format, spectra_locations, stack_size):
    ...
    random_locations = random.sample(
        spectra_locations, min(stack_size, len(spectra_locations))
    )
...
def get_batch_01(index_list, data_set, bucket_id):
    ...
    for index in index_list:
        ...
        (spectrum_holder, candidate_intensity_list, decoder_input) = data_set[
            bucket_id
        ][index]
    ...
    for length_idx in xrange(decoder_size):
        ...
        batch_intensity_inputs.append(
            np.array(
                [
                    candidate_intensity_lists[batch_idx][length_idx]
...
def get_batch_2(index_list, data_set, bucket_id):
    ...
    for index in index_list:
        ...
        spectra_list.append(spectrum_holder)
        candidate_intensity_lists_forward.append(
            candidate_intensity_list_forward
        )
        candidate_intensity_lists_backward.append(
            candidate_intensity_list_backward
        )
    ...
    for length_idx in xrange(decoder_size):
        ...
        batch_intensity_inputs_forward.append(
            np.array(
                [
                    candidate_intensity_lists_forward[batch_idx][length_idx]
                    for batch_idx in xrange(batch_size)
                ],
                dtype=np.float32,
            )
        ...
        batch_intensity_inputs_backward.append(
            np.array(
                [
                    candidate_intensity_lists_backward[batch_idx][length_idx]
                    for batch_idx in xrange(batch_size)
                ],
                dtype=np.float32,
            )
        ...
        batch_decoder_inputs_forward.append(
            np.array(
                [
                    decoder_inputs_forward[batch_idx][length_idx]
                    for batch_idx in xrange(batch_size)
                ],
                dtype=np.int32,
            )
        ...
        batch_decoder_inputs_backward.append(
            np.array(
                [
                    decoder_inputs_backward[batch_idx][length_idx]
                    for batch_idx in xrange(batch_size)
                ],
                dtype=np.int32,
            )
...
def print_AA_basic(
    ...
    decoder_input_AA = [
        deepnovo_config.vocab_reverse[x] for x in decoder_input
    ]

    output_seq, output_score = output
    output_len = (
        output_seq.index(LAST_LABEL) if LAST_LABEL in output_seq else 0
    )
    output_AA = [
        deepnovo_config.vocab_reverse[x] for x in output_seq[:output_len]
    ]
...
def test_AA_match_1by1(decoder_input, output):
    ...
    while index_aa < decoder_input_len:
        ...
        if (
            abs(
                deepnovo_config.mass_ID[decoder_input[index_aa]]
...
def test_AA_decode_batch(
    scans, decoder_inputs, outputs, direction, output_file_handle
):
    """TODO(nh2tran): docstring."""
    ...
    for index in xrange(len(scans)):
        ...
        (accuracy_AA, len_AA, len_decode, exact_match, len_match) = (
            test_AA_decode_single(decoder_input, output_seq, direction)
        )
...
def test_logit_single_01(decoder_input, output_logit):
    ...
    return test_AA_true_feeding_single(
        decoder_input, output, deepnovo_config.FLAGS.direction
    )


def test_logit_single_2(
    decoder_input_forward,
    decoder_input_backward,
    ...
    return test_AA_true_feeding_single(
        decoder_input_forward, output, direction=0
    )
...
def test_random_accuracy(sess, model, data_set, bucket_id):
    ...
    for _ in xrange(num_step):
        ...
        start_time = time.time()
        random_index_list = random.sample(
            xrange(data_set_len), deepnovo_config.batch_size
        )

        # get_batch_01/2
        if (
            deepnovo_config.FLAGS.direction == 0
            or deepnovo_config.FLAGS.direction == 1
        ):
            (
                encoder_inputs,
                intensity_inputs,
                decoder_inputs,
                target_weights,
            ) = get_batch_01(random_index_list, data_set, bucket_id)
        else:
            (
                encoder_inputs,
                ...
                decoder_inputs_backward,
                target_weights,
            ) = get_batch_2(random_index_list, data_set, bucket_id)
        ...
        if deepnovo_config.FLAGS.direction == 0:
            ...
            _, step_loss, output_logits_forward, output_logits_backward = (
                model.step(
                    sess,
                    encoder_inputs,
        ...
        if (
            ...
            (
                ...
                num_exact_match,
                num_len_match,
            ) = test_logit_batch_01(decoder_inputs, output_logits)
        else:
            (
                batch_accuracy_AA,
                batch_len_AA,
...
def test_accuracy(sess, model, data_set, bucket_id, print_summary=True):
    ...
    for chunk in data_set_index_chunk_list:
        ...
        if (
            ...
            (
                ...
                decoder_inputs,
                target_weights,
            ) = get_batch_01(chunk, data_set, bucket_id)
        else:
            (
                encoder_inputs,
                intensity_inputs_forward,
        ...
        if deepnovo_config.FLAGS.direction == 0:
            ...
            _, loss, output_logits_forward, output_logits_backward = (
                model.step(
                    sess,
                    encoder_inputs,
                    intensity_inputs_forward=intensity_inputs_forward,
        ...
        avg_loss += loss * len(
            chunk
        )  # because the loss was averaged by batch_size
    ...
    if print_summary:
        ...
        print(
            "  bucket %d avg_accuracy_AA %.4f" % (bucket_id, avg_accuracy_AA)
        )
        print(
            "  bucket %d avg_accuracy_peptide %.4f"
            % (bucket_id, avg_accuracy_peptide)
        )
        print(
            "  bucket %d avg_accuracy_len %.4f" % (bucket_id, avg_accuracy_len)
        )
...
def knapsack_build():
    ...
    peptide_mass = peptide_mass - (
        deepnovo_config.mass_C_terminus + deepnovo_config.mass_H
    )
    print("peptide_mass = ", peptide_mass)

    peptide_mass_round = int(
        round(peptide_mass * deepnovo_config.KNAPSACK_AA_RESOLUTION)
    )
    ...
    peptide_mass_upperbound = (
        peptide_mass_round + deepnovo_config.KNAPSACK_AA_RESOLUTION
    )

    knapsack_matrix = np.zeros(
        shape=(deepnovo_config.vocab_size, peptide_mass_upperbound), dtype=bool
    )

    for aa_id in xrange(
        3, deepnovo_config.vocab_size
    ):  # excluding PAD, GO, EOS
...
def knapsack_search(knapsack_matrix, peptide_mass, mass_precision_tolerance):
    ...
    candidate_AA_id = np.flatnonzero(
        np.any(
            knapsack_matrix[
                :,
                peptide_mass_lowerbound_col : peptide_mass_upperbound_col + 1,
...
def knapsack_search_mass(
    knapsack_matrix, peptide_mass, mass_precision_tolerance
):
    """TODO(nh2tran): docstring."""
    ...
    if m_nonzero_col.size == 0:
        return []
    m_closest_col = m_nonzero_col[
        np.argmin(np.absolute(m_nonzero_col - mass_precision_tolerance))
    ]
    m_closest_col_mass = (
        peptide_mass_lowerbound_col + m_closest_col + 1
...
def decode_true_feeding_01(sess, model, direction, data_set):
    ...
    data_set_index_list = range(data_set_len)
    data_set_index_stack_list = [
        data_set_index_list[i : i + decode_stack_size]
    ...
    for stack in data_set_index_stack_list:
        ...
        stack_c_state0, stack_h_state0 = sess.run(
            fetches=output_feed, feed_dict=input_feed
        )
    ...
    for stack_index, stack in enumerate(data_set_index_stack_list):
        ...
        for index in xrange(len(data_set[0][2])):
            ...
            block_candidate_intensity = np.array(
                [data_set[x][1][index] for x in stack]
            )
            block_AA_id_2 = np.array(
                [data_set[x][2][index] for x in stack]
            )  # nobi
            if index - 1 >= 0:
                block_AA_id_1 = np.array(
                    [data_set[x][2][index - 1] for x in stack]
                )
            else:
            ...
            input_feed[model.input_dict["intensity"].name] = (
...
def decode_true_feeding_2(sess, model, data_set):
    ...
    output_log_prob_forwards = decode_true_feeding_01(
        sess, model, direction=0, data_set=data_set_forward
    )

    output_log_prob_backwards = decode_true_feeding_01(
        sess, model, direction=1, data_set=data_set_backward
    )
...
def decode_true_feeding(sess, model, data_set):
    ...
    if (
        ...
        output_log_probs = decode_true_feeding_01(
            sess, model, deepnovo_config.FLAGS.direction, data_set
        )

        spectrum_time = time.time() - spectrum_time

        decoder_inputs = [x[-1] for x in data_set]
        (batch_accuracy_AA, batch_len_AA, num_exact_match, num_len_match) = (
            test_logit_batch_01(zip(*decoder_inputs), output_log_probs)
        )
    else:

        (output_log_prob_forwards, output_log_prob_backwards) = (
            decode_true_feeding_2(sess, model, data_set)
        )
    ...
    return (
        batch_accuracy_AA / batch_len_AA,
...
def decode_beam_search_01(
    ...
    data_set_index_list = range(data_set_len)
    data_set_index_stack_list = [
        data_set_index_list[i : i + decode_block_size]
    ...
    for spectrum_id in xrange(min(decode_block_size, len(prefix_mass_list))):

        active_search.append([])
        active_search[-1].append(spectrum_id)
        active_search[-1].append(
            [
                [
                    [FIRST_LABEL],  # current_paths
    ...
    while True:
        ...
        for entry in active_search:
            ...
            for path in current_paths:
                ...
                if AA_ID_2 == LAST_LABEL:  # nobi
                    if (
                        abs(prefix_mass - peptide_mass)
                        <= deepnovo_config.PRECURSOR_MASS_PRECISION_TOLERANCE
                    ):
                        output_top_paths[spectrum_id].append(
                            [path[0], path[2], direction]
                        )
                    continue
                ...
                # CANDIDATE INTENSITY
                candidate_intensity = get_candidate_intensity(
                    spectrum_original, peptide_mass, prefix_mass, direction
                )
                ...
                mass_filter_candidate = knapsack_search(
                    knapsack_matrix, suffix_mass, knapsack_precision
                )

                if not mass_filter_candidate:  # not enough mass left to extend
                    mass_filter_candidate.append(
                        LAST_LABEL
                    )  # try to end the sequence
        ...
        if block_AA_ID_1:
            ...
            input_feed = {}
            input_feed[model.input_dict["AAid"][0].name] = (
                block_AA_ID_1  # nobi
            )
            input_feed[model.input_dict["AAid"][1].name] = (
                block_AA_ID_2  # nobi
            )
            input_feed[model.input_dict["intensity"].name] = (
                block_candidate_intensity
        ...
        for entry_index, entry in enumerate(active_search):
            ...
            for index in xrange(
                block_index, block_index + entry_block_size[entry_index]
            ):
                ...
                for aa_id in block_mass_filter_candidate[index]:
                    ...
                    new_paths[-1].append(
                        block_prefix_mass[index]
                        + deepnovo_config.mass_ID[aa_id]
                    )

                    if aa_id > 2:  # do NOT add score of GO, EOS, PAD
                        new_paths[-1].append(
                            block_score[index] + current_log_prob[index][aa_id]
                        )
                    else:
                        new_paths[-1].append(block_score[index])

                    new_paths[-1].append(
                        current_c_state[index]
                    )  # lstm.len_full
                    new_paths[-1].append(
                        current_h_state[index]
                    )  # lstm.len_full
            ...
            if len(new_paths) > deepnovo_config.FLAGS.beam_size:
                ...
                top_k_indices = np.argpartition(
                    -new_path_scores, deepnovo_config.FLAGS.beam_size
                )[
                    : deepnovo_config.FLAGS.beam_size
                ]  # pylint: disable=line-too-long
        ...
        if (
            active_search_len < decode_block_size
            and spectrum_count < data_set_len
        ):

            new_spectrum_count = min(
                spectrum_count + decode_block_size - active_search_len,
                data_set_len,
            )
...
def decode_beam_search_2(sess, model, data_set, knapsack_matrix):
    ...
    for spectrum_id in xrange(data_set_len):
        ...
        peptide_mass_C_location = int(
            round(peptide_mass_C * deepnovo_config.SPECTRUM_RESOLUTION)
        )
        spectrum_forward = data_set[spectrum_id][2]
        argmax_location = np.argpartition(
            -spectrum_forward[:peptide_mass_C_location], num_position
        )[
            :num_position
        ]  # pylint: disable=line-too-long
    ...
    for candidate_mass in candidate_mass_list:
        ...
        for spectrum_id in xrange(data_set_len):
            ...
            if (not top_paths_forward[spectrum_id]) or (
                not top_paths_backward[spectrum_id]
            ):  # any list is empty
                continue
            else:
...
def decode_beam_search(
    sess, model, data_set, knapsack_matrix, output_file_handle
):
    """TODO(nh2tran): docstring."""
    ...
    if (
        ...
        output_top_paths = decode_beam_search_01(
            sess,
            model,
            knapsack_matrix,
        ...
        outputs = decode_beam_select_01(
            output_top_paths, deepnovo_config.FLAGS.direction
        )
        ...
        output_top_paths = decode_beam_search_2(
            sess, model, data_set, knapsack_matrix
        )
...
def decode(input_file=deepnovo_config.decode_test_file):
    ...
    with tf.Session() as sess:
        ...
        spectra_file_location = inspect_file_location(
            deepnovo_config.data_format, input_file
        )
        ...
        if deepnovo_config.FLAGS.beam_search:
            ...
            # print to output file
            decode_output_file = (
                deepnovo_config.FLAGS.train_dir + "/decode_output.tab"
            )
            with open(decode_output_file, "w") as output_file_handle:
                print(
                    "scan\ttarget_seq\toutput_seq\toutput_score\taccuracy_AA\tlen_AA"
                    "\texact_match\n",
                    file=output_file_handle,
                    end="",
                )
                ...
                with open(input_file, "r") as input_file_handle:
                    ...
                    for stack in spectra_file_location_stack_list:
                        ...
                        print(
                            "Read {0:d}/{1:d} spectra, reading time = {2:.2f}".format(
                                counter_peptide,
                                data_set_len,
            ...
            print(
                "  precision_AA %.4f" % (total_accuracy_AA / total_len_decode)
            )
            print(
                "  recall_peptide %.4f"
            ...
            print(
                "  recall_len %.4f" % (total_len_match / total_peptide_decode)
            )
            print(
                "  spectrum_time %.4f"
                % (total_spectrum_time / total_peptide_decode)
            )
        ...
        # DECODE with TRUE FEEDING
            ...
            for bucket_id in xrange(len(deepnovo_config._buckets)):
                ...
                (
                    ...
                    num_len_match,
                    spectrum_time,
                ) = decode_true_feeding(sess, model, data_set[bucket_id])
...
def train_cycle(
    ...
    train_bucket_sizes = [
        len(train_set[b]) for b in xrange(len(deepnovo_config._buckets))
    ]
    ...
    while True:
        ...
        bucket_id = min(
            [
                i
                for i in xrange(len(train_buckets_scale))
        ...
        random_index_list = random.sample(
            xrange(train_bucket_sizes[bucket_id]), deepnovo_config.batch_size
        )
        ...
        if (
            ...
            (
                ...
                decoder_inputs,
                target_weights,
            ) = get_batch_01(random_index_list, train_set, bucket_id)
        else:
            (
                encoder_inputs,
                intensity_inputs_forward,
                ...
                decoder_inputs_backward,
                target_weights,
            ) = get_batch_2(random_index_list, train_set, bucket_id)
        ...
        step_time[0] += (
            time.time() - start_time
        ) / deepnovo_config.steps_per_checkpoint
        ...
        if current_step[0] % deepnovo_config.steps_per_checkpoint == 0:
            ...
            if (
                ...
                (batch_accuracy_AA, batch_len_AA, num_exact_match, _) = (
                    test_logit_batch_01(decoder_inputs, output_logits)
                )
            else:
                (batch_accuracy_AA, batch_len_AA, num_exact_match, _) = (
            ...
            print(
                "global step %d epoch %.1f step-time %.2f perplexity %.4f"
            ...
            for bucket_id in valid_bucket_pos_id:
                ...
                eval_loss, accuracy_AA, accuracy_peptide, _ = test_accuracy(
                    sess, model, valid_set, bucket_id, print_summary=False
                )
            ...
            valid_ppx = (
                math.exp(valid_loss) if valid_loss < 300 else float("inf")
            )
...
def train():
    ...
    spectra_file_location_train = inspect_file_location(
        deepnovo_config.data_format, deepnovo_config.input_file_train
    )
    spectra_file_location_valid = inspect_file_location(
        deepnovo_config.data_format, deepnovo_config.input_file_valid
    )
    ...
    with tf.Session() as sess:
        ...
        # Open log_file
        log_file = (
            deepnovo_config.FLAGS.train_dir + "/log_file_caption_2dir.tab"
        )
        print("Open log_file: ", log_file)
        log_file_handle = open(log_file, "w")
        print(
            "global step \t epoch \t step-time \t"
        ...
        checkpoint_path = os.path.join(
            deepnovo_config.FLAGS.train_dir, "translate.ckpt"
        )
        ...
        while True:
            ...
            if epoch >= deepnovo_config.epoch_stop:
                print(
                    "EPOCH: {0:.1f}, EXCEED {1:d}, STOP TRAINING LOOP".format(
                        epoch, deepnovo_config.epoch_stop
                    )
                )
                break
...
def test_true_feeding():
    ...
    spectra_file_location_test = inspect_file_location(
        deepnovo_config.data_format, deepnovo_config.input_file_test
    )
